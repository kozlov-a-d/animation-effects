...
(0, _triangles2.default)({
    ...
    //     width: 1024,    
    //     height: 768,
    //     polylines: [246.3,215.3,218.7,184.6,233.6,217.1,
...
var _arrayUtility = __webpack_require__(4);
...
var drawTriangle = function drawTriangle(ctx, x1, y1, x2, y2, x3, y3) {
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x2, y2);
    ctx.lineTo(x3, y3);
...
var transformTriangle = function transformTriangle(elements, containerWidth, containerHeight) {
    elements.forEach(function (item) {
        ...
        for (var _i = 0; _i < item.polylines.length; _i += 2) {
            item.polylines[_i] = item.polylines[_i] - element.x.min + (containerWidth - element.width) / 2;
            item.polylines[_i + 1] = item.polylines[_i + 1] - element.y.min + (containerHeight - element.height) / 2;
        }
    });
};
...
function triangles(options) {
    ...
    self.elements.forEach(function (item) {
        var voidValue = (self.width + self.height) / 4;
        item.polylines = _arrayUtility.arrayUtility.padArray(item.polylines, 102 * 6, voidValue);
    });
    ...
    var controllerAnimationTime = gui.add(settings, 'animationTime', 0, 5);
    var controllerAnimationEase = gui.add(settings, 'animationEase', ['elastic', 'linear', 'back', 'bounce']);
    var controllerAnimationEaseX = gui.add(settings, 'animationEaseX', 0, 3).step(0.1);
    var controllerAnimationEaseY = gui.add(settings, 'animationEaseY', 0, 3).step(0.1);
    ...
    function restartTimeline(tl) {
        ...
        tl.play(0);
        tl.clear();
        tl.to(state, settings.animationTime, self.elements[1].polylines).to(state, settings.animationTime, self.elements[0].polylines, settings.animationTime + 0.5);
    }
    ...
    function render(state) {
        ...
        for (var i = state.length; i >= 0; i -= 6) {
